...
import static com.team254.lib.trajectory.TrajectoryConfig.Strategy.STEP;
import static com.team254.lib.trajectory.TrajectoryConfig.Strategy.S_CURVE;
import static com.team254.lib.trajectory.TrajectoryConfig.Strategy.TRAPEZOIDAL;
...
public class TrajectoryGenerator {
   ...
   * @param initialVel The starting velocity (WARNING: May be ignored)
   * @param initialHeading The starting heading
   * @param goalPos The goal position
   * @param goalVel The goal velocity (WARNING: May be ignored)
  ...
  public static Trajectory generate(
    ...
    if (strategy == TrajectoryConfig.Strategy.AUTOMATIC) {
      strategy = TrajectoryConfig.chooseStrategy(initialVel, goalVel, config.getMaxVelocity());
    }

    Trajectory traj;

    if (strategy == TrajectoryConfig.Strategy.STEP) {
        traj = generateStepTrajectory(config, goalPos);
    } else if (strategy == TrajectoryConfig.Strategy.TRAPEZOIDAL) {
        traj = generateTrapezoidalTrajectory(config, initialVel, goalPos, goalVel);
    } else if (strategy == TrajectoryConfig.Strategy.S_CURVE) {
        traj = generateSCurveStrategy(config, goalPos);
    } else {
        return null;
    }

    setHeadings(traj, initialHeading, goalHeading);

    return traj;
  }

  public static Trajectory generateStepTrajectory(RobotConfig config, double goal_pos){
        Trajectory traj;
        double timeToTraverse = goal_pos / config.getMaxVelocity();
        double impulse = timeToTraverse / config.getTimeInterval();

        // Round down, meaning we may undershoot by less than max_vel*dt (a tiny amount)
        // This is due to discretization and avoids a strange final velocity.
    ...
    public static Trajectory generateTrapezoidalTrajectory(
            RobotConfig config,
            double start_vel,
            double goal_pos,
            double goal_vel) {
    ...
    public static Trajectory generateSCurveStrategy(
            RobotConfig config,
            double goal_pos) {
      Trajectory traj;
        // How fast can we go given maximum acceleration and deceleration?
    ...
    public static void setHeadings(Trajectory traj, double startHeading, double goalHeading){
        ...
        for (int i = 0; i < traj.getNumSegments(); ++i) {
            traj.segments[i].heading = startHeading + total_heading_change
                    * (traj.segments[i].pos)
